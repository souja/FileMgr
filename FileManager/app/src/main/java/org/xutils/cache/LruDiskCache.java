...
public final class LruDiskCache {
    ...
    private LruDiskCache(String dirName) {
        ...
        this.cacheDir = getXCachePath(dirName);
    ...
    public LruDiskCache setMaxSize(long maxSize) {
        if (maxSize > 0L) {
            long diskFreeSize = getDiskAvailableSize();
        ...
        trimExecutor.execute(new Runnable() {
            ...
            public void run() {
                if (available) {
                    ...
                    // trim disk
                    try {
                        while (getFileOrDirSize(cacheDir) > diskCacheSize) {
                            List<DiskCacheEntity> rmList = cacheDb.selector(DiskCacheEntity.class)
    ...
    private File getXCachePath(String dirName) {
        String path = Environment.getExternalStorageDirectory()
                + File.separator + "com.ihisun.ymyk/xcache/" + dirName;
        File file = new File(path);
        if (!file.exists()) {
            file.mkdirs();
        }
     ...
     * @return byte 单位 kb
     */
    private long getDiskAvailableSize() {
