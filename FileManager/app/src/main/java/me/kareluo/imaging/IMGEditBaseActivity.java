...
abstract class IMGEditBaseActivity extends Activity implements View.OnClickListener,
    ...
    private void initViews() {
        ...
        chooseSizeView = findViewById(R.id.layout_size);
    }
    ...
    public void onClick(View v) {
        ...
        if (vid == R.id.rb_doodle) {
            ...
            onRotateClipClick();
        } else if (vid == R.id.tv_set) {
            chooseSizeView.setVisibility(View.VISIBLE);
        } else if (vid == R.id.tv_sizeA) {//1:1
            if (sizeMode == 1) {
                chooseSizeView.setVisibility(View.GONE);
                return;
            }
            sizeMode = 1;
            mImgView.reSize();
            chooseSizeView.setVisibility(View.GONE);
            onModeClick(IMGMode.CLIP, sizeMode);

        } else if (vid == R.id.tv_sizeB) {//2:1
            if (sizeMode == 2) {
                chooseSizeView.setVisibility(View.GONE);
                return;
            }
    ...
    public abstract void onModeClick(IMGMode mode, int sizeMode);
